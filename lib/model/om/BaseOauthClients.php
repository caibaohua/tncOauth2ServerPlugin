 ...
 */
abstract class BaseOauthClients extends myBaseObject  implements Persistent {


  const PEER = 'OauthClientsPeer';
	...
	/**
	 * Initializes internal state of BaseOauthClients object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     OauthClients The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(OauthClientsPeer::CLIENT_ID))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->client_id = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->client_secret = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->redirect_uri = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OauthClientsPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseOauthClients:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOauthClients:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOauthClients:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseOauthClients:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			OauthClientsPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setClientSecret($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setRedirectUri($arr[$keys[2]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OauthClientsPeer::CLIENT_ID)) $criteria->add(OauthClientsPeer::CLIENT_ID, $this->client_id);
		if ($this->isColumnModified(OauthClientsPeer::CLIENT_SECRET)) $criteria->add(OauthClientsPeer::CLIENT_SECRET, $this->client_secret);
		if ($this->isColumnModified(OauthClientsPeer::REDIRECT_URI)) $criteria->add(OauthClientsPeer::REDIRECT_URI, $this->redirect_uri);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseOauthClients:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseOauthClients::%s', $method));
    }

 ...
 */
abstract class BaseOauthRefreshTokens extends myBaseObject  implements Persistent {


  const PEER = 'OauthRefreshTokensPeer';
	...
	/**
	 * Initializes internal state of BaseOauthRefreshTokens object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     OauthRefreshTokens The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(OauthRefreshTokensPeer::REFRESH_TOKEN,OauthRefreshTokensPeer::CLIENT_ID,OauthRefreshTokensPeer::USER_ID,OauthRefreshTokensPeer::EXPIRES,OauthRefreshTokensPeer::SCOPE))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->refresh_token = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->client_id = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->user_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->expires = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OauthRefreshTokensPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseOauthRefreshTokens:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOauthRefreshTokens:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOauthRefreshTokens:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseOauthRefreshTokens:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			OauthRefreshTokensPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setRefreshToken($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setClientId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUserId($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setExpires($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OauthRefreshTokensPeer::REFRESH_TOKEN)) $criteria->add(OauthRefreshTokensPeer::REFRESH_TOKEN, $this->refresh_token);
		if ($this->isColumnModified(OauthRefreshTokensPeer::CLIENT_ID)) $criteria->add(OauthRefreshTokensPeer::CLIENT_ID, $this->client_id);
		if ($this->isColumnModified(OauthRefreshTokensPeer::USER_ID)) $criteria->add(OauthRefreshTokensPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(OauthRefreshTokensPeer::EXPIRES)) $criteria->add(OauthRefreshTokensPeer::EXPIRES, $this->expires);
		if ($this->isColumnModified(OauthRefreshTokensPeer::SCOPE)) $criteria->add(OauthRefreshTokensPeer::SCOPE, $this->scope);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseOauthRefreshTokens:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseOauthRefreshTokens::%s', $method));
    }

 ...
 */
abstract class BaseOauthAccessTokens extends myBaseObject  implements Persistent {


  const PEER = 'OauthAccessTokensPeer';
	...
	/**
	 * Initializes internal state of BaseOauthAccessTokens object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     OauthAccessTokens The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(OauthAccessTokensPeer::ACCESS_TOKEN,OauthAccessTokensPeer::CLIENT_ID,OauthAccessTokensPeer::USER_ID,OauthAccessTokensPeer::EXPIRES,OauthAccessTokensPeer::SCOPE))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->access_token = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OauthAccessTokensPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseOauthAccessTokens:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOauthAccessTokens:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOauthAccessTokens:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseOauthAccessTokens:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			OauthAccessTokensPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setAccessToken($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OauthAccessTokensPeer::ACCESS_TOKEN)) $criteria->add(OauthAccessTokensPeer::ACCESS_TOKEN, $this->access_token);
		if ($this->isColumnModified(OauthAccessTokensPeer::CLIENT_ID)) $criteria->add(OauthAccessTokensPeer::CLIENT_ID, $this->client_id);
		if ($this->isColumnModified(OauthAccessTokensPeer::USER_ID)) $criteria->add(OauthAccessTokensPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(OauthAccessTokensPeer::EXPIRES)) $criteria->add(OauthAccessTokensPeer::EXPIRES, $this->expires);
		if ($this->isColumnModified(OauthAccessTokensPeer::SCOPE)) $criteria->add(OauthAccessTokensPeer::SCOPE, $this->scope);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseOauthAccessTokens:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseOauthAccessTokens::%s', $method));
    }
